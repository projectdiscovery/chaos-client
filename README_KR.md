...
| 플래그 (Flag)              | 설명                                         | 예시                                                       |
|---------------------------|----------------------------------------------|------------------------------------------------------------|
| `-d`                      | 서브도메인을 찾을 대상 도메인                | `chaos -d uber.com`                                        |
| `-count`                  | 지정한 도메인의 통계 정보 출력                | `chaos -d uber.com -count`                                 |
| `-o`                      | 출력 결과를 저장할 파일 (선택 사항)           | `chaos -d uber.com -o uber.txt`                            |
| `-json`                   | 결과를 JSON 형식으로 출력                     | `chaos -d uber.com -json`                                  |
| `-key`                    | Chaos API 키 지정                            | `chaos -key API_KEY`                                       |
| `-dL`                     | 도메인 목록이 포함된 파일 (선택 사항)         | `chaos -dL domains.txt`                                    |
| `-silent`                 | 출력을 최소화하여 깔끔하게 표시               | `chaos -d uber.com -silent`                                |
| `-version`                | 현재 chaos 클라이언트 버전 출력               | `chaos -version`                                           |
| `-verbose`                | 상세한 출력 표시                              | `chaos -verbose`                                           |
| `-update`                 | 최신 버전으로 업데이트                        | `chaos -up`                                                |
| `-disable-update-check`   | 자동 업데이트 확인 비활성화                   | `chaos -duc`                                               |
...
## 라이브러리로 사용하기
`Chaos`는 CLI뿐 아니라 라이브러리 형태로도 사용 가능합니다. `Options` 구조체를 통해 동일한 방식으로 설정 후 호출합니다.
...
위 프로그램을 실행하려면 `CHAOS_KEY` 환경 변수가 설정되어 있어야 합니다.
...
여러분의 기여 덕분에 커뮤니티는 계속 성장하고 있습니다. 감사합니다! :heart: